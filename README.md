...
- `filepattern` - Optional file glob pattern (defaults to `*` for all files)

If you omit the file pattern, the filter applies to all files. If you omit the name, the regex becomes the name.
...
# Content filters with file targeting
...
- `@filter=name:regex:filepattern` - Set default content filter with optional file targeting
